...
Generated: 2026-09-07T13:45:00.000Z
...
The authority is research/forward-archive on the Git default branch. Content-addressed records and append-only operation logs survive process exit, restart, Render replacement and deploy. Scheduled and manual GitHub workflow runs write this same authority.
...
Status includes last morning/afternoon/manual runs, next expected run, health, capture counts, missed/deferred work, provider failures, request counts, cache hits and actual hours-to-kickoff distribution. Current health: STALE.
...
**SCHEDULE_PUBLISHED_AWAITING_SECRET_VERIFICATION_AND_FIRST_RUN**. The workflow is published on the default branch. Provider-secret presence and the first actual 7 AM/2 PM scheduled executions still require observation before Phase 7 can be declared fully operational.

...
const summary={header:{league:{slug:'nfl'},week:1,competitions:[{id:'401',competitors:[
...
const teams={sports:[{leagues:[{teams:[{team:{id:'6',displayName:event.awayTeam,name:'Cowboys',abbreviation:'DAL'}},{team:{id:'21',displayName:event.homeTeam,name:'Eagles',abbreviation:'PHI'}}]}]}]};
...
const official:any={report:async()=>({version:'test',season:2026,week:1,source:'https://www.nfl.com/injuries/league/2026/reg1',fetchedAt:new Date(now).toISOString(),status:'PARTIAL_SUCCESS',
  teams:['Cowboys','Eagles'],rows:[{team:'Cowboys',player:'Test Player',position:'QB',injury:'Ankle',practiceStatus:'Limited Participation in Practice',gameStatus:'Questionable'}],note:'fixture'})};
const teamResearch:any={context:async(team:string,opponent:string)=>({version:'test',status:'PARTIAL_SUCCESS',source:'https://github.com/nflverse/nflverse-data',fetchedAt:new Date(now).toISOString(),team,opponent,current:null,
  prior:{season:2025,games:17,throughWeek:18,passEpaPerAttempt:.1,rushEpaPerCarry:0,passYardsPerAttempt:7,rushYardsPerCarry:4,passEpaAllowedPerAttempt:.05,rushEpaAllowedPerCarry:-.02,sacksPerGame:2,qbHitsPerGame:5},
  note:'fixture',modelAdjustmentEnabled:false})};
...
test('NFL preflight invokes real source adapters, stores exact IDs and reports expected QBs without claiming active status',async()=>{
  const root=fs.mkdtempSync(path.join(os.tmpdir(),'nfl-context-ok-')),calls:string[]=[];try{const service=new NflContextIngestion(root,research(calls),getter(calls),()=>now,async()=>{},official,teamResearch),result=await service.refresh([event]);
    ...
    assert.equal(result.games[0].availability.status,'NO_PROVIDER_CONFIGURED');assert.equal(result.games[0].weather.windMph,8);assert.equal(result.games[0].teams[0].injuries.listed,1);
    assert.equal(result.games[0].teams[0].officialInjuryReport.listed,1);assert.equal(result.summary.officialInjuryTeams,2);assert.equal(result.summary.teamResearchLoaded,2);assert.equal(result.storage.storeStatus,'SUCCESS');
...
test('one NFL team source failure is precise and does not erase the other team or weather',async()=>{
  const root=fs.mkdtempSync(path.join(os.tmpdir(),'nfl-context-isolate-'));try{const calls:string[]=[],get=async(url:string)=>{calls.push(url);if(url.includes('/teams/6/roster'))throw Error('NFL data source returned HTTP 429');
    ...
    const result=await new NflContextIngestion(root,research(),get,()=>now,async()=>{},official,teamResearch).refresh([event]),away=result.games[0].teams[0],home=result.games[0].teams[1];
...
test('NFL team directory keeps roster and depth diagnostics available when game summary is offline',async()=>{
  const root=fs.mkdtempSync(path.join(os.tmpdir(),'nfl-context-fallback-'));try{const r:any={matchEvent:async()=> '401',summary:async()=>{throw Error('NFL data source returned HTTP 503');}},
    result=await new NflContextIngestion(root,r,getter(),()=>now,async()=>{},official,teamResearch).refresh([event]);
...
test('NFL identity mismatch fails closed while preserving an auditable report',async()=>{
  const root=fs.mkdtempSync(path.join(os.tmpdir(),'nfl-context-match-'));try{const bad={...summary,header:{...summary.header,competitions:[{...summary.header.competitions[0],competitors:[
    ...
    const r:any={matchEvent:async()=> '401',summary:async()=>bad},result=await new NflContextIngestion(root,r,getter(),()=>now,async()=>{},official,teamResearch).refresh([event]);

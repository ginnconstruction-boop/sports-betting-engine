...
async function main() {
  ...
  const compact = (row: ReturnType<typeof evaluateNflGameLineSeason>) => ({ season:row.season,config:row.config,games:row.games,excluded:row.excluded,
    modelMargin:row.modelMargin,naiveMargin:row.naiveMargin,marketMargin:row.marketMargin,paperAts:row.paperAts,disagreementBuckets:row.disagreementBuckets });
  const report = { protocol:'nfl-full-game-margin-research-v1', createdAt:new Date().toISOString(), source:'nflverse schedules CC-BY-4.0',

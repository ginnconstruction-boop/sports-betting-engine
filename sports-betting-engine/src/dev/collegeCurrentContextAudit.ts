...
import {resolveCollegeTeamContext} from '../services/collegeContextEvidence';
...
async function main(){
  ...
  try{
    ...
    const concise=(team:any)=>({team:team.teamName,completeness:team.completeness,reliability:team.reliability,qb:team.qb.status,roster:team.sections.roster.status,
      ...
      talentDepth:team.sections.talentDepth.status,fcsTier:team.fcsTier.value??'UNKNOWN_FCS',injuries:team.sections.injuries.status,weather:team.sections.weather.status,
      weatherValues:team.weather});
    const summary=games.map((g:any)=>{const away=resolveCollegeTeamContext(refreshed.records,{teamId:g.away.teamId,teamName:g.away.teamName,season,eventId:g.eventId,asOf:forecastAt,currentGames:0}),
      home=resolveCollegeTeamContext(refreshed.records,{teamId:g.home.teamId,teamName:g.home.teamName,season,eventId:g.eventId,asOf:forecastAt,currentGames:0});
      return{game:`${g.away.teamName} @ ${g.home.teamName}`,eventId:g.eventId,kickoff:g.kickoff,away:concise(away),home:concise(home)};});
    console.log(JSON.stringify({date:compact,source:url,retrievedAt:new Date(retrieved).toISOString(),sportsbookOddsCalls:0,records:refreshed.total,warnings:refreshed.warnings,games:summary},null,2));

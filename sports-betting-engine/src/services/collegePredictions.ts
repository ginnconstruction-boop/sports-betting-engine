...
export const COLLEGE_MODEL_LIMITATIONS='Experimental paper only. No verified injury, starting-QB, transfer/roster, weather or pace adjustment. Prior-season evidence dominates early in the season. No stake recommendation or real wager.';
...
export class CollegePredictions {
  ...
  constructor(private paper:NflPaperLedger,root:string,private get=fetchNflJson,private now=()=>Date.now(),private load=loadCollegeModelBundle){
  ...
  readiness(){const b=this.load(),calibration=JSON.parse(fs.readFileSync(path.resolve(__dirname,'../data/college-score-audit-details.json'),'utf8'));
    return{version:b.payload.version,bundleHash:b.sha256,validation:b.payload.validation,oddsAudit:b.payload.oddsAudit,calibration,limitations:COLLEGE_MODEL_LIMITATIONS};}
  ...
  async scan(rows:any[],raw:RawEvent[],trackPaper:boolean){
    const recommendations:any[]=[],projections:any[]=[],warnings:string[]=[];
    ...
    if(!eligible.length)return{recommendations,projections,modelReadiness:readiness,recommendationStatus:'no_verified_upcoming_games',warnings};
    ...
    for(const row of eligible){
      ...
      const {selected,assessed}=selectCollegeQuotes(row.event,quotes,p,b,this.now(),b.validation.paperApproved);
      const item={event:row.event,projection:p,selected:trackPaper?undefined:selected,
        reason:selected.length?(trackPaper?'Eligible experimental paper spread.':'Preview only; check the paper-tracking box and scan to save eligible selections.'):'No fresh approved-market quote passed the fixed paper thresholds.',
        quotesAssessed:assessed.length};
      ...
      for(const candidate of selected){
        try{
          const evidenceHash=this.archive.record({kind:'college_forecast_v1',event:row.event,identity:row.identity,projection:p,
            ...candidate,inputEvidenceHash:inputsHash,selectionVersion:COLLEGE_SELECTION_VERSION,rawOdds:event}).hash;
          const result=this.paper.saveCollegeModel(row.event,candidate.quote,row.identity,{projection:p,assessment:candidate.assessment,
            inputEvidenceHash:inputsHash,forecastEvidenceHash:evidenceHash,bundleHash:bundle.sha256,selectionVersion:COLLEGE_SELECTION_VERSION},b.validation);
          recommendations.push({...result,label:'EXPERIMENTAL PAPER — not a real-money betting recommendation'});
    ...
    return{recommendations,projections,modelReadiness:readiness,recommendationStatus:trackPaper?'experimental_paper':'preview_only',warnings};
  }
}

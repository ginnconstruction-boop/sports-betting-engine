...
export interface NflDailyJob {id:string;date:string;startedAt:string;finishedAt?:string;status:'running'|'complete'|'partial'|'failed';stage:'discovering'|'context'|'grading'|'finished';
  preflight:any|null;warnings:string[];grading:{gamesPlanned:number;gamesChecked:number;picksChecked:number;sourceFailures:number;pending:number;unableToGrade:number;legacyReview:number;voids:number;review:number};}
export class NflDailyRun {
  ...
  start(){if(this.active)return this.get(this.active.id);for(const id of [...this.jobs.keys()].slice(0,Math.max(0,this.jobs.size-9)))this.jobs.delete(id);
    const job:NflDailyJob={id:randomUUID(),date:collegeDate(this.deps.now()),startedAt:new Date(this.deps.now()).toISOString(),status:'running',stage:'discovering',preflight:null,warnings:[],
      grading:{gamesPlanned:0,gamesChecked:0,picksChecked:0,sourceFailures:0,pending:0,unableToGrade:0,legacyReview:0,voids:0,review:0}};this.jobs.set(job.id,job);this.active=job;
  ...
  private async execute(job:NflDailyJob){let events:any[]=[];try{events=(await this.deps.events()).filter(event=>collegeDate(Date.parse(event.commenceTime))===job.date);job.stage='context';job.preflight=await this.deps.preflight(events);
    ...
    job.stage='grading';try{const eligible=this.deps.read().filter(p=>p.event.sportKey==='americanfootball_nfl'&&['PENDING','REVIEW','UNABLE_TO_GRADE'].includes(p.result)&&Date.parse(p.event.commenceTime)+4*3600_000<this.deps.now());
      ...
      const picks=this.deps.read();job.grading.pending=picks.filter(p=>p.result==='PENDING').length;job.grading.unableToGrade=picks.filter(p=>p.result==='UNABLE_TO_GRADE').length;
      job.grading.legacyReview=picks.filter(p=>p.result==='REVIEW').length;job.grading.review=job.grading.unableToGrade+job.grading.legacyReview;job.grading.voids=picks.filter(p=>p.result==='VOID').length;

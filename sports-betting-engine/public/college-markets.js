...
function renderCollegeDiagnostic(panel,row){
  ...
  if(s.currentContext){for(const side of ['away','home']){const c=s.currentContext[side],team=side==='away'?row.event.awayTeam:row.event.homeTeam;
    nflText(d,`CURRENT CONTEXT — ${team}: unweighted field availability ${c.unweightedFieldCoverage??'unavailable'}%; reliability ${c.reliability}; QB ${c.qb.starter??'unknown'} — ${c.qb.status}; current-season data ${c.sections.currentSeason?.status??'missing'}; returning production ${c.sections.returningProduction.status}; transfers ${c.sections.transfers.status}; coaching ${c.sections.coaching.status}; talent/depth ${c.sections.talentDepth.status}; injuries ${c.sections.injuries.status}; weather ${c.sections.weather.status}.`);
    if(c.dataCompleteness){const q=c.dataCompleteness;nflText(d,`DATA COMPLETENESS — ${team}: critical ${q.critical.available}/${q.critical.total} available (${q.critical.partial} partial); high ${q.high.available}/${q.high.total} available (${q.high.partial} partial); medium ${q.medium.available}/${q.medium.total} available (${q.medium.partial} partial). Category counts only; no model weights.`);}
    ...
    for(const [category,pipeline] of Object.entries(c.pipelineDiagnostics??{}))nflText(debug,`${category}: source ${pipeline.sourceResult}; `+Object.entries(pipeline.stages).map(([stage,result])=>`${stage}=${result}`).join(' · ')+`; valid/fresh fields ${pipeline.validFields}/${pipeline.freshFields} of ${pipeline.requiredFields}.`);

...
function renderNflDaily(job){
  ...
  if(data.predictiveBoard)renderNflPredictiveBoard(panel,data.predictiveBoard);
  nflText(panel,`Context coverage: ${data.summary.rostersLoaded}/${data.summary.teams} rosters; ${data.summary.expectedQbs}/${data.summary.teams} expected depth-chart QBs; official weekly injury-report teams ${data.summary.officialInjuryTeams}/${data.summary.teams}; team/opponent research ${data.summary.teamResearchLoaded}/${data.summary.teams}; legacy display completeness ${data.summary.averageCompleteness}%; verified official game-day availability ${data.summary.officialAvailability}/${data.summary.teams}. The preflight does not refit a model, place a bet, or save a new paper pick.`);
...
function nflBoardHeading(parent,tag,text,className=''){const el=document.createElement(tag);el.textContent=text;if(className)el.className=className;parent.append(el);return el;}
function nflBoardOpportunity(parent,row,label){if(!row){nflText(parent,`${label}: MARKET UNAVAILABLE`);return;}const prefix=row.grade>=60&&row.recommendable?'RECOMMENDED BOARD':'MODEL VIEW — BELOW 60';nflText(parent,`${label}: ${row.label} · model ${row.projection??'n/a'} · difference ${row.difference??'n/a'} · ${row.grade}/100 ${row.tier} · ${row.price==null?'price unavailable':`${row.price>0?'+':''}${row.price} at ${row.sportsbook}`}. ${prefix}.`);}
function renderNflPredictiveBoard(parent,board){
  const wrap=document.createElement('section');wrap.className='nfl-predictive-board';nflBoardHeading(wrap,'h2','NFL PREDICTIVE BOARD');
  nflText(wrap,`${board.gradeMeaning} Only opportunities graded 60 or higher appear in the ranked recommendation sections.`);
  const top=document.createElement('div');top.className='nfl-top-board';nflBoardHeading(top,'h3','TOP 5 OVERALL (60+ ONLY)');
  if(!board.topOverall.length)nflText(top,'No opportunity reached 60. The game forecasts below are still available for your own judgment.');
  else board.topOverall.forEach((row,index)=>nflText(top,`${index+1}. ${row.label} — ${row.grade}/100 ${row.tier}${row.price==null?'':` · ${row.price>0?'+':''}${row.price}${row.sportsbook?` at ${row.sportsbook}`:''}`}`));wrap.append(top);
  const best=document.createElement('div');best.className='nfl-best-grid';for(const [type,label] of [['SPREAD','BEST SPREAD'],['MONEYLINE','BEST MONEYLINE'],['TOTAL','BEST TOTAL'],['PLAYER_PROP','BEST PROP'],['TEAM_TOTAL','BEST TEAM TOTAL'],['SGP','BEST SGP'],['PARLAY','BEST PARLAY']]){const row=board.bestByType[type];if(row)nflText(best,`${label}: ${row.label} — ${row.grade}/100 ${row.tier}`);}if(best.children.length)wrap.append(best);
  for(const game of board.games){const card=document.createElement('article');card.className='nfl-game-prediction';nflBoardHeading(card,'h3',game.matchup);
    if(game.forecast.status==='PROJECTED'){const score=document.createElement('div');score.className='nfl-predicted-score';score.textContent=`PREDICTED SCORE: ${game.matchup.split(' @ ')[0]} ${game.forecast.awayScore} — ${game.matchup.split(' @ ')[1]} ${game.forecast.homeScore}`;card.append(score);nflText(card,`Projected winner: ${game.forecast.winner} · ${game.projectedWinnerStrength} · home margin ${game.forecast.homeMargin>=0?'+':''}${game.forecast.homeMargin} · projected total ${game.forecast.total}.`);nflText(card,`Game script: ${game.gameScript}`);}else nflText(card,`FORECAST UNAVAILABLE: ${game.forecast.reason}`);
    if(game.bestGameAngle)nflText(card,`BEST GAME ANGLE: ${game.bestGameAngle.label} — ${game.bestGameAngle.grade}/100 ${game.bestGameAngle.tier}${game.bestGameAngle.grade<60?' (not on recommendation board)':''}.`);
    nflBoardOpportunity(card,game.spread,'SPREAD');nflBoardOpportunity(card,game.moneyline,'MONEYLINE');nflBoardOpportunity(card,game.total,'TOTAL');
    if(game.teamTotals.length){nflBoardHeading(card,'h4','TEAM TOTALS');for(const row of game.teamTotals)nflBoardOpportunity(card,row,'TEAM TOTAL');}
    if(game.playerProps.length){nflBoardHeading(card,'h4','PLAYER PROJECTIONS');for(const row of game.playerProps.slice(0,8))nflBoardOpportunity(card,row,'PROP');}
    if(game.sgps.length){nflBoardHeading(card,'h4','SUPPORTED SGP');for(const [index,row] of game.sgps.entries()){const legs=row.label.startsWith('2-leg')?'BEST 2-LEG SGP':row.label.startsWith('3-leg')?'BEST 3-LEG SGP':'SUPPORTED SGP';nflText(card,`${index===0?'BEST CONSERVATIVE SGP / ':''}${legs}: ${row.label} — ${row.grade}/100 ${row.tier}. Grade is not probability.`);}}
    const reason=game.bestGameAngle?.why??[];if(reason.length)nflText(card,`WHY: ${reason.slice(0,4).join(' ')}`);nflText(card,`DATA QUALITY: ${game.dataQuality}. KEY RISKS: ${game.keyRisks.join(' ')||'No additional recorded risk.'}`);
    const detail=document.createElement('details'),summary=document.createElement('summary');summary.textContent='Market, price, and research diagnostics';detail.append(summary);nflText(detail,`Evidence captured ${nflDisplayTime(game.capturedAt)} from ${game.source.replaceAll('_',' ').toLowerCase()}. Fresh prices are required for the recommendation board.`);nflText(detail,`Forecast: ${game.forecast.version}. Reconciliation: ${game.forecast.reconciliation}`);for(const derivative of game.derivatives)nflText(detail,`${derivative.market}: ${derivative.status} — ${derivative.reason}`);card.append(detail);wrap.append(card);}
  const audit=document.createElement('details'),title=document.createElement('summary');title.textContent='Grade distribution and safeguards';audit.append(title);nflText(audit,`60–69: ${board.gradeDistribution['60-69']} · 70–77: ${board.gradeDistribution['70-77']} · 78–84: ${board.gradeDistribution['78-84']} · 85–89: ${board.gradeDistribution['85-89']} · 90–100: ${board.gradeDistribution['90-100']} · below 60: ${board.below60}.`);for(const line of board.diagnostics)nflText(audit,line);nflText(audit,`Collection ${board.collection.mode}; scheduled ${board.collection.scheduled}; decision-layer API impact ${board.collection.newPaidProviderCalls} calls / ${board.collection.newPaidCredits} credits.`);wrap.append(audit);parent.prepend(wrap);
}
